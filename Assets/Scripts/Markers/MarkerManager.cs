...
namespace Markers
{
	...
	public class MarkerManager : Singleton<MarkerManager>
	{
		...
		public bool AllChecked => Markers.All(marker => marker.IsChecked);
		...
		private void Update()
		{
			// Only Editable in Pause
			if (GameManager.Instance.State != GameManager.GameState.Paused) return;

			// SHIFT => DELETE Mode
			if (ShiftDown) EditMarkerMode = EditMarkerMode.Delete;
			else if (ShiftUp) EditMarkerMode = prevMarkerMode;
		}
		...
		private void HandleOnGameStateChanged(GameManager.GameState newState) =>
			EditMarkerMode =
				newState == GameManager.GameState.Paused ? EditMarkerMode.Add : EditMarkerMode.None;
		...
		public Marker AddMarker(Vector2 normalizedPos, int index = -1)
		{
			...
			Marker marker = markersStorage.Add(normalizedPos, label, index);
			marker.State = state;
		...
		public Marker AddMarkerBetweenSelectedPair(Vector2 normalizedPos)
		{
			...
			int index = markersStorage.SelectedIndexPair!.Item2;
			Marker marker = AddMarker(normalizedPos, index);
		...
		{
			Marker marker = markersStorage.Remove(index);
		...
		public Marker ToggleSelectMarker(int index)
		{
			...
			bool isSelected = markersStorage.markers[index].Selected;
			bool twoSelected = markersStorage.SelectedCount == 2;
			bool notAdyacentToSelected =
			...
			Marker marker = markersStorage.markers[index];
		...
		{
			int collisionIndex = FindIndex(normalizedPos);
		...
		{
			int selectedIndex = markersStorage.SelectedIndex;

			Marker moved = MoveMarker(selectedIndex, targetPos);
