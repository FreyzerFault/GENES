...
public class BezierCurve : MonoBehaviour
{
    ...
    private void OnDrawGizmos()
    {
        ...
        if (points.Length > 1)
        {
            for (var i = 0; i < points.Length - 1; i++) DrawCurve(points[i], points[i + 1], resolution);
    ...
    /// </returns>
    public BezierPoint[] GetAnchorPoints() => (BezierPoint[])points.Clone();
    ...
    /// </param>
    public static Vector3 GetLinearPoint(Vector3 p1, Vector3 p2, float t) => p1 + (p2 - p1) * t;
    ...
    private static int BinomialCoefficient(int i, int n) => Factoral(n) / (Factoral(i) * Factoral(n - i));
