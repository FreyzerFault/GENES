...
namespace UnityEngine.UI.Extensions
{
    ...
    public class TextPic : TMP_Text, IPointerClickHandler, IPointerExitHandler, IPointerEnterHandler, ISelectHandler
#else
...
#endif
    {
        ...
        [SerializeField] private HrefClickEvent m_OnHrefClick = new();
        ...
        /// </summary>
        private static readonly Regex s_HrefRegex = new(@"<a href=([^>\n\s]+)>(.*?)(</a>)", RegexOptions.Singleline);
        ...
        protected void UpdateQuadImage()
        {
            ...
            if (matches != null && matches.Count > 0)
                for (var i = 0; i < matches.Count; i++)
                {
                    ...
                    if (m_ImagesPool.Count == 0) GetComponentsInChildren(true, m_ImagesPool);
                ...
                for (var i = m_ImagesPool.Count - 1; i > 0; i--)
                    if (m_ImagesPool[i])
            ...
            for (var i = m_ImagesPool.Count - 1; i >= matches.Count; i--)
            ...
            if (culled_ImagesPool.Count > 0) clearImages = true;
        }
        ...
        protected string GetOutputText()
        {
            ...
            if (href_matches != null && href_matches.Count > 0)
                for (var i = 0; i < href_matches.Count; i++)
                {
                    ...
                    if (isCreating_m_HrefInfos)
                    {
                        var hrefInfo = new HrefInfo
                        {
                            ...
                            endIndex = usesNewRendering
                                ? s_TextBuilder.Length + href_matches[i].Groups[2].Length - 1
                                : (s_TextBuilder.Length + href_matches[i].Groups[2].Length - 1) * 4 + 3,
                            name = group.Value
                        };
                    ...
                    else
                    {
                        if (count <= m_HrefInfos.Count - 1)
                        {
                            ...
                            m_HrefInfos[count].startIndex =
                                usesNewRendering ? s_TextBuilder.Length : s_TextBuilder.Length * 4;
                            m_HrefInfos[count].endIndex = usesNewRendering
                                ? s_TextBuilder.Length + href_matches[i].Groups[2].Length - 1
                                : (s_TextBuilder.Length + href_matches[i].Groups[2].Length - 1) * 4 + 3;
            ...
            if (isCreating_m_HrefInfos) isCreating_m_HrefInfos = false;
            ...
            for (var r = 0; r < matches.Count; r++) indexes.Add(matches[r].Index);
#endif
            ...
            if (matches != null && matches.Count > 0)
                for (var i = 0; i < matches.Count; i++)
                {
                    ...
                    if (usesNewRendering)
                    {
                        ...
                        for (var r = 0; r < removeCharacters.Count; r++)
                        ...
                        for (var r = 0; r < i; r++) charactersRemoved += matches[r].Length - 1;
            ...
            if (usesNewRendering)
                if (m_HrefInfos != null && m_HrefInfos.Count > 0)
                    for (var i = 0; i < m_HrefInfos.Count; i++)
                    {
                        ...
                        for (var r = 0; r < removeCharacters.Count; r++)
                        {
                            if (removeCharacters[r].Index < m_HrefInfos[i].startIndex
                                && !indexes.Contains(removeCharacters[r].Index))
                                startCharactersRemoved += removeCharacters[r].Length;
                            else if (removeCharacters[r].Index < m_HrefInfos[i].startIndex
                                     && indexes.Contains(removeCharacters[r].Index))
                                startCharactersRemoved += removeCharacters[r].Length - 1;

                            if (removeCharacters[r].Index < m_HrefInfos[i].endIndex
                                && !indexes.Contains(removeCharacters[r].Index))
                                endCharactersRemoved += removeCharacters[r].Length;
                            else if (removeCharacters[r].Index < m_HrefInfos[i].endIndex
                                     && indexes.Contains(removeCharacters[r].Index))
        ...
        protected override void OnPopulateMesh(VertexHelper toFill)
        {
            ...
            for (var i = 0; i < m_ImagesVertexIndex.Count; i++)
            {
                ...
                if (endIndex < toFill.currentVertCount)
                {
                    ...
                    positions.Add(
                        new Vector2(vert.position.x + fontSize / 2, vert.position.y + fontSize / 2) + imageOffset
                    );
            ...
            for (var h = 0; h < m_HrefInfos.Count; h++)
            {
                m_HrefInfos[h].boxes.Clear();

                if (m_HrefInfos[h].startIndex >= toFill.currentVertCount) continue;
        ...
        public void OnPointerClick(PointerEventData eventData)
        {
            ...
            for (var h = 0; h < m_HrefInfos.Count; h++)
            for (var i = 0; i < m_HrefInfos[h].boxes.Count; ++i)
        ...
        public void OnPointerEnter(PointerEventData eventData)
        {
            ...
            if (m_ImagesPool.Count >= 1)
                for (var i = 0; i < m_ImagesPool.Count; i++)
        ...
        protected override void OnEnable()
        {
            ...
            if (Application.unityVersion.StartsWith("2019.1."))
            {
                ...
                {
                    var number = Convert.ToInt32(Application.unityVersion[7].ToString());

                    if (number > 4) usesNewRendering = true;
                }
                else
                {
        ...
        private void LateUpdate()
        {
            ...
            lock (thisLock)
            {
                ...
                if (clearImages)
                {
                    for (var i = 0; i < culled_ImagesPool.Count; i++) DestroyImmediate(culled_ImagesPool[i]);
