...
namespace UnityEngine.UI.Extensions
{
    ...
    public class CUIImage : CUIGraphic
    {
        ...
        {
            if (_type == Image.Type.Sliced) return SlicedImageCornerRefVertexIdx;
        ...
        [Tooltip("For changing the size of the corner for tiled or sliced Image")] [HideInInspector] [SerializeField]
        public Vector2 cornerPosRatio = Vector2.one * -1; // -1 means unset value

        [HideInInspector] [SerializeField] protected Vector2 oriCornerPosRatio = Vector2.one * -1;
        ...
        {
            if (uiGraphic == null) uiGraphic = GetComponent<Image>();
        ...
        protected override void modifyVertices(List<UIVertex> _verts)
        {
            ...
            else if (UIImage.type == Image.Type.Sliced || UIImage.type == Image.Type.Tiled)
            {
                ...
                if (cornerPosRatio == Vector2.one * -1)
                {
                    ...
                    cornerPosRatio.x = (cornerPosRatio.x + rectTrans.pivot.x * rectTrans.rect.width)
                                       / rectTrans.rect.width;
                    cornerPosRatio.y = (cornerPosRatio.y + rectTrans.pivot.y * rectTrans.rect.height)
                ...
                if (cornerPosRatio.x < 0) cornerPosRatio.x = 0;
                if (cornerPosRatio.x >= 0.5f) cornerPosRatio.x = 0.5f;
                if (cornerPosRatio.y < 0) cornerPosRatio.y = 0;
                if (cornerPosRatio.y >= 0.5f) cornerPosRatio.y = 0.5f;
                ...
                for (var index = 0; index < _verts.Count; index++)
                {
                    ...
                    var horRatio = (uiVertex.position.x + rectTrans.rect.width * rectTrans.pivot.x)
                                   / rectTrans.rect.width;
                    ...
                    if (horRatio < oriCornerPosRatio.x)
                        ...
                        horRatio = Mathf.Lerp(
                            1 - cornerPosRatio.x,
                            1,
                            (horRatio - (1 - oriCornerPosRatio.x)) / oriCornerPosRatio.x
                        );
                    else
                        horRatio = Mathf.Lerp(
                            cornerPosRatio.x,
                            1 - cornerPosRatio.x,
                            (horRatio - oriCornerPosRatio.x) / (1 - oriCornerPosRatio.x * 2)
                        );
                    ...
                    if (verRatio < oriCornerPosRatio.y)
                        ...
                        verRatio = Mathf.Lerp(
                            1 - cornerPosRatio.y,
                            1,
                            (verRatio - (1 - oriCornerPosRatio.y)) / oriCornerPosRatio.y
                        );
                    else
                        verRatio = Mathf.Lerp(
                            cornerPosRatio.y,
                            1 - cornerPosRatio.y,
                            (verRatio - oriCornerPosRatio.y) / (1 - oriCornerPosRatio.y * 2)
                        );
