    ...
    public class UILineRenderer : UIPrimitiveBase
    {
        ...
        protected override void OnPopulateMesh(VertexHelper vh)
        {
            ...
            if (m_segments != null && m_segments.Count > 0)
            {
                ...
                for (var s = 0; s < m_segments.Count; s++)
                {
                    var pointsToDraw = m_segments[s];
        ...
        protected override void GeneratedUVs()
        {
            ...
            middleUvs = new[]
                { UV_TOP_CENTER_LEFT, UV_BOTTOM_CENTER_LEFT, UV_BOTTOM_CENTER_RIGHT, UV_TOP_CENTER_RIGHT };
        ...
        private int GetSegmentPointCount()
        {
            ...
            {
                var pointCount = 0;
                foreach (var segment in Segments) pointCount += segment.Length;
                return pointCount;
            }
        ...
        {
            if (segmentIndex > 0) return Segments[segmentIndex - 1][index - 1];
        ...
        public Vector2 CalculatePointOnCurve(float t, Vector2 p1, Vector2 p2, Vector2 p3, Vector2 p4)
        {
            ...
            var x = p1.x + (-p1.x * 3 + t * (3 * p1.x - p1.x * t)) * t +
                    (3 * p2.x + t * (-6 * p2.x + p2.x * 3 * t)) * t +
                    (p3.x * 3 - p3.x * 3 * t) * t2 + p4.x * t3;

            var y = p1.y + (-p1.y * 3 + t * (3 * p1.y - p1.y * t)) * t +
                    (3 * p2.y + t * (-6 * p2.y + p2.y * 3 * t)) * t +
        ...
        {
            var from_p1_to_p3 = p3 - p1;
            var from_p1_to_p2 = p2 - p1;
            var dot = Vector2.Dot(from_p1_to_p3, from_p1_to_p2.normalized);
