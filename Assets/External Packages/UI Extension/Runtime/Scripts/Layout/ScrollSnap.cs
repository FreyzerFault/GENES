...
namespace UnityEngine.UI.Extensions
{
    ...
    public class ScrollSnap : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollSnap
    {
        ...
        private readonly int _fastSwipeTarget = 10;
        ...
        private void LateUpdate()
        {
            ...
            if (_lerp)
            {
                ...
                _listContainerTransform.localPosition = Vector3.Lerp(
                    _listContainerTransform.localPosition,
                    _lerpTarget,
                    7.5f * Time.deltaTime
                );
        ...
        public void UpdateListItemsSize()
        {
            ...
            if (LinkScrolrectScrollSensitivity) _scroll_rect.scrollSensitivity = _itemSize;
                ...
                if (direction == ScrollDirection.Horizontal)
                    foreach (var tr in _listContainerTransform)
                    {
                        ...
                        if (child.activeInHierarchy)
                        {
                            ...
                            if (childLayout == null) childLayout = child.AddComponent<LayoutElement>();
        ...
        public void UpdateListItemPositions()
        {
            if (!_listContainerRectTransform.rect.size.Equals(_listContainerCachedSize))
            {
                ...
                if (_itemsCount != activeCount) PageChanged(CurrentPage());

                _itemsCount = activeCount;
                _listContainerCachedSize.Set(
                    _listContainerRectTransform.rect.size.x,
        ...
        {
            if (direction == ScrollDirection.Horizontal)
                _scroll_rect.horizontalNormalizedPosition = _pages > 1 ? _startingPage / (float)(_pages - 1) : 0;
            else
                _scroll_rect.verticalNormalizedPosition =
                    _pages > 1 ? (_pages - _startingPage - 1) / (float)(_pages - 1) : 0;
        }
        ...
        private void UpdateScrollbar(bool linkSteps)
        {
            if (linkSteps)
            {
                ...
                {
                    if (_scroll_rect.verticalScrollbar != null) _scroll_rect.verticalScrollbar.numberOfSteps = _pages;
                }
            }
            else
            {
                if (direction == ScrollDirection.Horizontal)
                {
                    if (_scroll_rect.horizontalScrollbar != null) _scroll_rect.horizontalScrollbar.numberOfSteps = 0;
                }
                else
                {
                    if (_scroll_rect.verticalScrollbar != null) _scroll_rect.verticalScrollbar.numberOfSteps = 0;
                }
            }
        }
        ...
        private void PageChanged(int currentPage)
        {
            ...
            if (NextButton) NextButton.interactable = currentPage < _pages - 1;
