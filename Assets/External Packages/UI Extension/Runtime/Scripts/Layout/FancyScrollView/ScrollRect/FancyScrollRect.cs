...
namespace UnityEngine.UI.Extensions
{
        ...
        where TContext : class, IFancyScrollRectContext, new()
    {
        ...
        ///     コンテンツ先頭の余白.
        /// </summary>
        [SerializeField] protected float paddingHead;

        /// <summary>
        ///     コンテンツ末尾の余白.
        /// </summary>
        [SerializeField] protected float paddingTail;
        ...
        private void OnScrollerValueChanged(float p)
        {
            ...
            if (Scroller.Scrollbar)
            {
                ...
                else if (p < 0f) ShrinkScrollbar(-p);
            }
        }
        ...
        /// <param name="alignment">ビューポート内におけるセル位置の基準. 0f(先頭) ~ 1f(末尾).</param>
        /// <param name="onComplete">移動が完了した際に呼び出されるコールバック.</param>
        protected virtual void ScrollTo(
            int index, float duration, Ease easing, float alignment = 0.5f, Action onComplete = null
        )
        {
        ...
        {
            var contentLength = Mathf.Max(
                ItemsSource.Count + (paddingHead + paddingTail - spacing) / (CellSize + spacing),
                1
            );
        ...
        protected float ToFancyScrollViewPosition(float position) =>
            position / Mathf.Max(ItemsSource.Count - 1, 1) * MaxScrollPosition - PaddingHeadLength;
        ...
        protected float ToScrollerPosition(float position) => (position + PaddingHeadLength) / MaxScrollPosition
