...
namespace UnityEngine.UI.Extensions
{
    ...
    {
        [SerializeField] [Tooltip("desired ScrollRect")]
        private ScrollRect scrollRect;

        [SerializeField] [Tooltip("Elements to populate inside the scroller")]
        private GameObject[] arrayOfElements;

        [SerializeField] [Tooltip("Center display area (position of zoomed content)")]
        private RectTransform center;

        [SerializeField] [Tooltip("Size / spacing of elements")]
        private RectTransform elementSize;

        [SerializeField] [Tooltip("Scale = 1/ (1+distance from center * shrinkage)")]
        private Vector2 elementShrinkage = new(1f / 200, 1f / 200);

        [SerializeField] [Tooltip("Minimum element scale (furthest from center)")]
        private Vector2 minScale = new(0.7f, 0.7f);

        [SerializeField] [Tooltip("Select the item to be in center on start.")]
        private int startingIndex = -1;

        [SerializeField] [Tooltip("Stop scrolling past last element from inertia.")]
        private bool stopMomentumOnEnd = true;

        [SerializeField] [Tooltip("Set Items out of center to not interactible.")]
        private bool disableUnfocused = true;

        [SerializeField] [Tooltip("Button to go to the next page. (optional)")]
        private GameObject scrollUpButton;

        [SerializeField] [Tooltip("Button to go to the previous page. (optional)")]
        ...
        [SerializeField] [Tooltip("Event fired when the focused item is Changed. (optional)")]
        ...
        public UIVerticalScroller(
            RectTransform center, RectTransform elementSize, ScrollRect scrollRect, GameObject[] arrayOfElements
        )
        {
            this.center = center;
        ...
        public void Awake()
        {
            ...
            if (arrayOfElements == null || arrayOfElements.Length == 0)
            {
                ...
                if (childCount > 0)
                {
                    ...
                    for (var i = 0; i < childCount; i++) arrayOfElements[i] = ScrollingPanel.GetChild(i).gameObject;
                }
            }
        }
        ...
        public void Start()
        {
            if (scrollUpButton) scrollUpButton.GetComponent<Button>().onClick.AddListener(() => ScrollUp());
            if (scrollDownButton) scrollDownButton.GetComponent<Button>().onClick.AddListener(() => ScrollDown());
        ...
        public void Update()
        {
            ...
            for (var i = 0; i < arrayOfElements.Length; i++)
            {
                ...
                //Magnifying effect
                var scale = Vector2.Max(
                    minScale,
                    new Vector2(1 / (1 + distance[i] * elementShrinkage.x), 1 / (1 + distance[i] * elementShrinkage.y))
                );
            ...
            var oldFocusedElement = FocusedElementIndex;

            for (var i = 0; i < arrayOfElements.Length; i++)
            {
                arrayOfElements[i].GetComponent<CanvasGroup>().interactable =
                    !disableUnfocused || minDistance == distance[i];
                if (minDistance == distance[i])
                {
                    FocusedElementIndex = i;
#if UNITY_2022_1_OR_NEWER
                    var textComponentTxtMeshPro = arrayOfElements[i].GetComponentInChildren<TMP_Text>();
                    if (textComponentTxtMeshPro != null) Result = textComponentTxtMeshPro.text;
#else
            ...
            if (FocusedElementIndex != oldFocusedElement) onFocusChanged?.Invoke(FocusedElementIndex);
            ...
            if (stopMomentumOnEnd
                && (arrayOfElements[0].GetComponent<RectTransform>().position.y > center.position.y
                    || arrayOfElements[arrayOfElements.Length - 1].GetComponent<RectTransform>().position.y
                    < center.position.y))
        ...
        public void UpdateChildren(int startingIndex = -1, GameObject[] arrayOfElements = null)
        {
            ...
            else
            {
                ...
                for (var i = 0; i < ScrollingPanel.childCount; i++)
            ...
            for (var i = 0; i < this.arrayOfElements.Length; i++)
            {
                ...
                var r = this.arrayOfElements[i].GetComponent<RectTransform>();
            ...
            if (startingIndex > -1)
            {
                startingIndex = startingIndex > this.arrayOfElements.Length
        ...
        {
            var newY = Mathf.Lerp(
                ScrollingPanel.anchoredPosition.y,
                ScrollingPanel.anchoredPosition.y + distReposition[FocusedElementIndex],
                Time.deltaTime * 2f
            );
            var newPosition = new Vector2(ScrollingPanel.anchoredPosition.x, newY);
            ScrollingPanel.anchoredPosition = newPosition;
        }

        public void SnapToElement(int element)
        {
            var deltaElementPositionY = elementSize.rect.height * element;
            var newPosition = new Vector2(ScrollingPanel.anchoredPosition.x, -deltaElementPositionY);
            ScrollingPanel.anchoredPosition = newPosition;
        }

        public void ScrollUp()
        {
            var deltaUp = elementSize.rect.height / 1.2f;
            var newPositionUp = new Vector2(
                ScrollingPanel.anchoredPosition.x,
                ScrollingPanel.anchoredPosition.y - deltaUp
            );
        ...
        {
            var deltaDown = elementSize.rect.height / 1.2f;
            var newPositionDown = new Vector2(
                ScrollingPanel.anchoredPosition.x,
                ScrollingPanel.anchoredPosition.y + deltaDown
            );
