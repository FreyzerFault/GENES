...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UIHorizontalScroller : MonoBehaviour
    {
        ...
        [SerializeField] [Tooltip("Select the item to be in center on start. (optional)")]
        ...
        public void Awake()
        {
            ...
            if (arrayOfElements == null || arrayOfElements.Length == 0)
            {
                ...
                if (childCount > 0)
                {
                    ...
                    for (var i = 0; i < childCount; i++) arrayOfElements[i] = scrollRect.content.GetChild(i).gameObject;
                }
            }
        }
        ...
        public void Start()
        {
            if (scrollLeftButton) scrollLeftButton.GetComponent<Button>().onClick.AddListener(() => ScrollLeft());

            if (scrollRightButton) scrollRightButton.GetComponent<Button>().onClick.AddListener(() => ScrollRight());
        ...
        public void Update()
        {
            ...
            if (stopMomentumOnEnd
                && (arrayOfElements[0].GetComponent<RectTransform>().position.x > center.position.x
                    || arrayOfElements[arrayOfElements.Length - 1].GetComponent<RectTransform>().position.x
                    < center.position.x))
        ...
        private void ScrollingElements()
        {
            var newX = Mathf.Lerp(
                ...
                ScrollingPanel.anchoredPosition.x + distReposition[FocusedElementIndex],
                Time.deltaTime * 2f
            );
            var newPosition = new Vector2(newX, ScrollingPanel.anchoredPosition.y);
            ScrollingPanel.anchoredPosition = newPosition;
        }

        public void SnapToElement(int element)
        {
            var deltaElementPositionX = elementSize.rect.width / 1.2f * element;
            var newPosition = new Vector2(-deltaElementPositionX, ScrollingPanel.anchoredPosition.y);
            ScrollingPanel.anchoredPosition = newPosition;
        }

        public void ScrollLeft()
        {
            var deltaLeft = elementSize.rect.width / 1.2f;
            var newPositionLeft = new Vector2(
                ScrollingPanel.anchoredPosition.x - deltaLeft,
        ...
        {
            var deltaRight =
                elementSize.rect.width / 1.2f; // arrayOfElements[0].GetComponent<RectTransform>().rect.width;
            var newPositionRight = new Vector2(
                ScrollingPanel.anchoredPosition.x + deltaRight,
                ScrollingPanel.anchoredPosition.y
            );
