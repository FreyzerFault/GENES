...
namespace UnityEngine.UI.Extensions
{
        ...
        IPointerClickHandler
    {
        ...
        private SelectionChangeStartEvent m_OnSelectionChangeStartEvent = new();

        [SerializeField] [Tooltip("Event fires as the page changes, while dragging or jumping")]
        private SelectionPageChangedEvent m_OnSelectionPageChangedEvent = new();

        [SerializeField] [Tooltip("Event fires when the page settles after a user has dragged")]
        private SelectionChangeEndEvent m_OnSelectionChangeEndEvent = new();
        ...
        public int CurrentPage
        {
            ...
            internal set
            {
                if (_isInfinite)
                {
                    ...
                    var infWindow = value / (float)_screensContainer.childCount;
                    ...
                    else if (value > _screensContainer.childCount - 1) value = value - _screensContainer.childCount;
                }
        ...
        private void OnValidate()
        {
            ...
            if (ScrollRectContent != null)
            {
                ...
                if (children != 0 || ChildObjects != null)
                {
                    ...
                    if (StartingScreen > childCount - 1) StartingScreen = childCount - 1;

                    if (StartingScreen < 0) StartingScreen = 0;
                }
            }

            if (MaskBuffer <= 0) MaskBuffer = 1;

            if (PageStep < 0) PageStep = 0;

            if (PageStep > 8) PageStep = 9;
        ...
        internal void InitialiseChildObjects()
        {
            if (ChildObjects != null && ChildObjects.Length > 0)
            {
                ...
                if (GetComponent<UI_InfiniteScroll>() != null) GetComponent<UI_InfiniteScroll>().Init();
            }
            else
            {
        ...
        internal void InitialiseChildObjectsFromScene()
        {
            ...
            for (var i = 0; i < childCount; i++)
            {
                ...
                if (MaskArea && ChildObjects[i].activeSelf) ChildObjects[i].SetActive(false);
            }
        }
        ...
        internal void InitialiseChildObjectsFromArray()
        {
            ...
            for (var i = 0; i < childCount; i++)
            {
                child = Instantiate(ChildObjects[i]);
        ...
        internal void UpdateVisible()
        {
            ...
            _halfNoVisibleItems = (int)Math.Round(_maskSize / (_childSize * MaskBuffer), MidpointRounding.AwayFromZero)
                                  / 2;
        ...
        internal void GetPositionforPage(int page, ref Vector3 target)
        {
            ...
            if (_isVertical)
            {
                _infiniteOffset = _screensContainer.anchoredPosition.y < 0
                    ...
                    : _screensContainer.sizeDelta.y * _infiniteWindow;
                _infiniteOffset = _infiniteOffset == 0 ? 0 :
                    _infiniteOffset < 0 ? _infiniteOffset - _childSize * _infiniteWindow :
        ...
        /// <summary>
        ///     Updates the _Lerp target to the closest page and updates the pagination bullets.  Each control's update loop will
        ...
        private void ToggleNavigationButtons(int targetScreen)
        {
            ...
            if (!_isInfinite)
            {
                if (PrevButton) PrevButton.GetComponent<Button>().interactable = targetScreen > 0;

                if (NextButton)
                    NextButton.GetComponent<Button>().interactable =
                        targetScreen < _screensContainer.transform.childCount - 1;
            }
        }
        ...
        public Transform CurrentPageObject() => _screensContainer.GetChild(CurrentPage);
