...
namespace UnityEngine.UI.Extensions
{
    ...
    public class ScrollPositionController : UIBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
    {
        ...
        [SerializeField] [Tooltip("Only used when inertia is enabled")]
        private Snap snap = new() { Enable = true, VelocityThreshold = 0.5f, Duration = 0.3f };
        ...
        private void Update()
        {
            ...
            if (autoScrollState.Enable)
            {
                ...
                else
                {
                    var alpha = Mathf.Clamp01(
                        (Time.unscaledTime - autoScrollState.StartTime) / Mathf.Max(
                            autoScrollState.Duration,
                            float.Epsilon
                        )
                    );
                    position = Mathf.Lerp(
                    ...
                    if (Mathf.Approximately(alpha, 1f)) autoScrollState.Reset();
                }
            ...
            if (currentScrollPosition != prevScrollPosition) prevScrollPosition = currentScrollPosition;
        }
        ...
        private float CalculateOffset(float position)
        {
            ...
            if (position > dataCount - 1) return dataCount - 1 - position;
        ...
        private void UpdatePosition(float position)
        {
            ...
            if (onUpdatePosition != null) onUpdatePosition(currentScrollPosition);
        }
        ...
        private int CalculateClosestIndex(int index)
        {
            ...
            if (Mathf.Abs(diff) > dataCount * 0.5f) diff = Mathf.Sign(-diff) * (dataCount - Mathf.Abs(diff));
