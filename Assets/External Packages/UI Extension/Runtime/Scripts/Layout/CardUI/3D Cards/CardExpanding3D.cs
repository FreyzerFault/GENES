...
namespace UnityEngine.UI.Extensions
{
    ...
    public class CardExpanding3D : MonoBehaviour
    {
        ...
        [SerializeField] private float cornerSize = 64;

        [Header("Parts")] public RectTransform[] cardCorners;

        public RectTransform[] cardEdges;
        public RectTransform cardCenter;

        [Header("Card Info")] [Tooltip("Positions and sizes card to its current transform.")]
        public bool cardAutoSize = true;

        public Vector2 cardSize;
        public Vector2 cardPosition;

        [Range(1, 96)] public int cardSuperness = 4;

        [Header("Page Info")] [Tooltip("Positions and sizes the page to the top third of the screen.")]
        public bool pageAutoSize = true;

        public Vector2 pageSize;
        public Vector2 pagePosition;

        [Range(1, 96)] public int pageSuperness = 96;

        private readonly Vector2[] nextCornerPos = new Vector2[4];
        private readonly Vector2[] nextEdgePos = new Vector2[4];
        private readonly Vector2[] nextEdgeScale = new Vector2[4];
        ...
        private void Start()
        {
            if (cardAutoSize)
            {
                cardSize = new Vector2(
                    cardCorners[0].localScale.x * 2 + cardEdges[0].localScale.x,
                    cardCorners[0].localScale.y * 2 + cardEdges[0].localScale.y
                );
        ...
        private void Update()
        {
            if (animationActive == 1 || animationActive == -1)
            {
                ...
                for (var i = 0; i < cardCorners.Length; i++)
                {
                    cardCorners[i].localPosition = Vector3.Lerp(
                        cardCorners[i].localPosition,
                        nextCornerPos[i],
                        Time.deltaTime * lerpSpeed
                    );

                    cardCorners[i].GetComponent<SuperellipsePoints>().superness = Mathf.Lerp(
                        cardCorners[i].GetComponent<SuperellipsePoints>().superness,
                        nextSuperness,
                        Time.deltaTime * lerpSpeed
                    );
                ...
                for (var i = 0; i < cardEdges.Length; i++)
                {
                    cardEdges[i].localPosition = Vector3.Lerp(
                        cardEdges[i].localPosition,
                        nextEdgePos[i],
                        Time.deltaTime * lerpSpeed
                    );
                    cardEdges[i].localScale = Vector3.Lerp(
                ...
                cardCenter.localScale = Vector3.Lerp(
        ...
        public void ToggleCard()
        {
            if (animationActive != 1 || animationActive == 0)
            {
                ...
                for (var i = 0; i < cardCorners.Length; i++)
                {
                    var posX = pageSize.x / 2 * Mathf.Sign(cardCorners[i].localScale.x) - cardCorners[i].localScale.x;
                    var posY = pageSize.y / 2 * Mathf.Sign(cardCorners[i].localScale.y) - cardCorners[i].localScale.y;
                ...
                for (var i = 0; i < cardEdges.Length; i++)
                {
                    ...
                    {
                        posX = Mathf.Sign(cardEdges[i].localPosition.x)
                               * (pageSize.x / 2 - cardEdges[i].localScale.x / 2);
                        posY = 0;
                    ...
                    {
                        posX = 0;
                        posY = Mathf.Sign(cardEdges[i].localPosition.y)
                               * (pageSize.y / 2 - cardEdges[i].localScale.y / 2);
            ...
            else if (animationActive != -1)
            {
                ...
                for (var i = 0; i < cardCorners.Length; i++)
                {
                    var posX = Mathf.Sign(cardCorners[i].localScale.x) * (cardSize.x / 2) - cardCorners[i].localScale.x;
                    var posY = Mathf.Sign(cardCorners[i].localScale.y) * (cardSize.y / 2) - cardCorners[i].localScale.y;
                ...
                for (var i = 0; i < cardEdges.Length; i++)
                {
                    ...
                    if (cardEdges[i].localPosition.x != 0)
                    {
                        posX = Mathf.Sign(cardEdges[i].localPosition.x) * (cardSize.x / 2)
                               - Mathf.Sign(cardEdges[i].localPosition.x) * (cardEdges[i].localScale.x / 2);
                        posY = 0;
                    ...
                    {
                        posX = 0;
                        posY = Mathf.Sign(cardEdges[i].localPosition.y) * (cardSize.y / 2)
                               - Mathf.Sign(cardEdges[i].localPosition.y) * (cardEdges[i].localScale.y / 2);
