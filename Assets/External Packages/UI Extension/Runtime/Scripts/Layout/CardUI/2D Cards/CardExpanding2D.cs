...
namespace UnityEngine.UI.Extensions
{
    public class CardExpanding2D : MonoBehaviour
    {
        ...
        [SerializeField] private Vector2 pageSize = Vector2.zero;
        private readonly Vector2 pageCenter = Vector2.zero;
        ...
        private void Update()
        {
            ...
            if (animationActive == 1)
            {
                ...
                if (rectTrans.offsetMin.x < pageMin.x * 0.995f && rectTrans.offsetMin.y < pageMin.y * 0.995f
                                                               && rectTrans.offsetMax.x > pageMax.x * 0.995f
                                                               && rectTrans.offsetMax.y > pageMax.y * 0.995f)
                {
                    rectTrans.offsetMin = pageMin;
                    rectTrans.offsetMax = pageMax;

                    ///Changes the button color so it's visible in the page view.
                    buttonRect.GetComponent<Image>().color = Color32.Lerp(
            ...
            else if (animationActive == -1)
            {
                buttonRect.GetComponent<Image>().color = Color32.Lerp(
                    ...
                    Time.deltaTime * lerpSpeed * 1.25f
                );
                ...
                if (rectTrans.offsetMin.x > cardMin.x * 1.005f && rectTrans.offsetMin.y > cardMin.y * 1.005f
                                                               && rectTrans.offsetMax.x < cardMax.x * 1.005f
                                                               && rectTrans.offsetMax.y < cardMax.y * 1.005f)
                {
