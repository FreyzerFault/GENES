    ...
    public class CardStack2D : MonoBehaviour
    {
            ...
            for (var i = cards.Length; i > -1; i--)
                if (i < cards.Length - 1)
                    cardPositions[i] = new Vector3(
                        -Mathf.Pow(2, i + xPowerDifference) + cardPositions[i + 1].x,
                        0,
        ...
        private void Update()
        {
            if (canUseHorizontalAxis)
            {
                ...
                if ((UIExtensionsInputManager.GetAxisRaw("Horizontal") < 0
                     || UIExtensionsInputManager.GetKey(leftButton)) && cardArrayOffset > 0)
                {
                    cardArrayOffset--;
                    StartCoroutine(ButtonCooldown());
                }
                else if ((UIExtensionsInputManager.GetAxisRaw("Horizontal") > 0
                          || UIExtensionsInputManager.GetKey(rightButton)) && cardArrayOffset < cards.Length - 1)
                {
                    cardArrayOffset++;
                    StartCoroutine(ButtonCooldown());
                }
            ...
            for (var i = 0; i < cards.Length; i++)
            {
                cards[i].localPosition = Vector3.Lerp(
                    ...
                    cardPositions[i + cardArrayOffset],
