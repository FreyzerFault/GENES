...
namespace UnityEngine.UI.Extensions
{
    ...
    public class FlowLayoutGroup : LayoutGroup
    {
        ...
        protected void LayoutRow(
            IList<RectTransform> contents, float rowWidth, float rowHeight, float maxWidth, float xOffset,
            float yOffset, int axis
        )
        {
            var xPos = xOffset;
            ...
            for (var j = 0; j < contents.Count; j++)
            {
                ...
                if (ChildForceExpandHeight) rowChildHeight = rowHeight;
                ...
                if (j < contents.Count - 1) xPos += rowChildWidth + SpacingX;
            }
        }

        protected void LayoutCol(
            IList<RectTransform> contents, float colWidth, float colHeight, float maxHeight, float xOffset,
            float yOffset, int axis
        )
        {
            var yPos = yOffset;
            ...
            for (var j = 0; j < contents.Count; j++)
            {
                ...
                if (j < contents.Count - 1) yPos += rowChildHeight + SpacingY;
            }
        }
