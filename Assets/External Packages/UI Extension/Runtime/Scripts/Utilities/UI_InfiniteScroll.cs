    ...
    public class UI_InfiniteScroll : MonoBehaviour
    {
        ...
        public virtual void SetNewItems(ref List<Transform> newItems)
        {
            if (_scrollRect != null)
            {
                ...
                for (var i = _scrollRect.content.childCount - 1; i >= 0; i--)
                {
                ...
                foreach (var newItem in newItems) newItem.SetParent(_scrollRect.content);
        ...
        private void SetItems()
        {
            ...
            foreach (RectTransform transform in _scrollRect.content.transform) transform.pivot = Vector3.zero;
        ...
        {
            if (_verticalLayoutGroup) _verticalLayoutGroup.enabled = false;
            if (_horizontalLayoutGroup) _horizontalLayoutGroup.enabled = false;
            if (_contentSizeFitter) _contentSizeFitter.enabled = false;
            if (_gridLayoutGroup) _gridLayoutGroup.enabled = false;
            ...
            for (var i = 0; i < items.Count; i++)
            {
                if (_isHorizontal)
                {
                    if (_scrollRect.transform.InverseTransformPoint(items[i].gameObject.transform.position).x
                        > items[i].sizeDelta.x + _threshold.x && items[i] == lastChild)
                    {
                        ...
                        lastChild.transform.SetAsFirstSibling();
                             ...
                             < -items[i].sizeDelta.x - _threshold.x - 100 && items[i] == firstChild)
                    {
                ...
                if (_isVertical)
                {
                    if (_scrollRect.transform.InverseTransformPoint(items[i].gameObject.transform.position).y
                        > items[i].sizeDelta.y + _threshold.y && items[i] == firstChild)
                    {
                        ...
                        firstChild.transform.SetAsLastSibling();
                             ...
                             < -items[i].sizeDelta.y - _threshold.y - 100 && items[i] == lastChild)
                    {
